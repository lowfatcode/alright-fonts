...
#include "pretty-poly/pretty-poly.h"
...
typedef struct  {
  ...
  float letting_spacing;            // spacing between characters (%)   
...
uint16_t  ru16(FILE* file) {uint8_t  w[2]; fread((char *) w, 1, 2, file); return  w[0] <<  8 |  w[1];}
int16_t   rs16(FILE* file) {uint8_t  w[2]; fread((char *) w, 1, 2, file); return  w[0] <<  8 |  w[1];}
uint32_t  ru32(FILE* file) {uint8_t dw[4]; fread((char *)dw, 1, 4, file); return dw[0] << 24 | dw[1] << 16 | dw[2] << 8 | dw[3];}
uint8_t    ru8(FILE* file) {return fgetc(file);}
int8_t     rs8(FILE* file) {return fgetc(file);}

bool af_load_font_file(FILE* file, af_face_t *face) {
  // check header magic bytes are present
  char marker[4]; fread(marker, 1, 4, file);
...
void af_render_glyph(af_glyph_t* glyph, af_text_metrics_t *tm) {
  ...
  poly.paths = AF_MALLOC(poly.count * sizeof(pp_path_t));
  for(int i = 0; i < poly.count; i++) {
    pp_path_t *path = &poly.paths[i];
    path->count = glyph->paths[i].point_count;
    path->points = AF_MALLOC(glyph->paths[i].point_count * sizeof(pp_point_t));
...
int get_line_width(af_face_t *face, wchar_t *text, af_text_metrics_t *tm) {
  ...
  for(wchar_t c = *text; text < end; text++, c = *text) {
    ...
    if(c == L' ') {
      ...
      line_width += (glyph->advance * tm->letting_spacing) / 100.0f;
    }
  }
  return line_width;
}
  ...
  while(end) {
    ...
    for(wchar_t c = *text; text < end; text++, c = *text) {
      ...
      if(c == L' ') {
        ...
        caret.x += (glyph->advance * tm->letting_spacing) / 100.0f;
      }
...
pp_rect_t af_measure(af_face_t *face, wchar_t *text, af_text_metrics_t *tm) {
  pp_rect_t result;
  bool first = true;
